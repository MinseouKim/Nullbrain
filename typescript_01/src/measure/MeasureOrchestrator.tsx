...
const STEPS: Step[] = [
  ...
  { id:"tpose",       title:"T-포즈 기준 수집",           instruction:"양팔을 좌우로 벌리고 손목이 어깨 이상." },
  { id:"side",        title:"측면 정렬 수집",             instruction:"옆을 보고 어깨-엉덩이-무릎-발목 한 라인." },
  { id:"ankle_rom",   title:"발목 ROM(도스플렉션) 측정",   instruction:"벽을 향해 무릎을 앞으로 밀었다가 돌아오기 2~3회." },
  { id:"squat",       title:"스쿼트 ROM",                 instruction:"천천히 앉았다 일어서기 2~3회." },
  { id:"elbow_flex",  title:"팔꿈치 ROM",                 instruction:"한쪽씩 완전 굽힘/신전 2~3회." },
  { id:"shoulder_abd",title:"어깨 외전 ROM",              instruction:"양팔 옆→머리 위까지 올렸다 내리기." },
  { id:"neck_rom",    title:"목 ROM",                     instruction:"숙임/뒤젖힘/좌우 기울임 최대 범위." },
...
const MeasureOrchestrator: React.FC<Props> = ({ heightCm, onDone }) => {
  ...
  const fullVisible  = (kp: KP) => REQUIRED_VISIBLE_GROUPS.fullCore .every(([a,b])=> visOK(kp,a)&&visOK(kp,b));
  const upperVisible = (kp: KP) => REQUIRED_VISIBLE_GROUPS.upperCore.every(([a,b])=> visOK(kp,a)&&visOK(kp,b));
  ...
  }, [angles, baseline, circ, cmPerPx, heightCm, lengths, onDone, posture, rom, symmetry]);

  const onPose = useCallback(({ kp, size }: { kp: KP; size: Size }) => {
    setFocusRoi(null);

    const nose = getPx(kp, LM.NOSE, size);
    const le = getPx(kp, LM.LEFT_EAR, size),  re = getPx(kp, LM.RIGHT_EAR, size);
    const ls = getPx(kp, LM.LEFT_SHOULDER, size), rs = getPx(kp, LM.RIGHT_SHOULDER, size);
    const lelb = getPx(kp, LM.LEFT_ELBOW, size), relb = getPx(kp, LM.RIGHT_ELBOW, size);
    const lw = getPx(kp, LM.LEFT_WRIST, size),  rw = getPx(kp, LM.RIGHT_WRIST, size);
    const lh = getPx(kp, LM.LEFT_HIP, size),    rh = getPx(kp, LM.RIGHT_HIP, size);
    const lk = getPx(kp, LM.LEFT_KNEE, size),   rk = getPx(kp, LM.RIGHT_KNEE, size);
    const la = getPx(kp, LM.LEFT_ANKLE, size),  ra = getPx(kp, LM.RIGHT_ANKLE, size);

    // 키 픽셀 샘플 → 중앙값 → EMA
    const headTopY = Math.min(nose?.y ?? Infinity, le?.y ?? Infinity, re?.y ?? Infinity);
    const ankleBottomY = Math.max(la?.y ?? -Infinity, ra?.y ?? -Infinity);
    const heightPx = (isFinite(headTopY) && isFinite(ankleBottomY)) ? (ankleBottomY - headTopY) : null;
    ...
    if (current.id === "full") {
      if (fullVisible(kp) && cmPerPx) {
        ...
        // 비대칭/골반폭
        if (lh && rh) {
          setBaseline(b => ({ ...(b ?? {}), pelvis_width_px: +Math.hypot(rh.x - lh.x, rh.y - lh.y).toFixed(1) }));
          setSymmetry(s => ({ ...(s ?? {}), pelvis_delta_px: +(rh.y - lh.y).toFixed(1) }));
        }
        if (ls && rs) {
          setSymmetry(s => ({ ...(s ?? {}), shoulder_delta_px: +(rs.y - ls.y).toFixed(1) }));
        }
        ...
        const lineAngle = (hip:Pt|null, ankle:Pt|null, knee:Pt|null)=>{
        ...
        if (le && re && ls && rs) {
          ...
          if (neckAng!=null) {
            setPosture(p => ({ ...(p ?? {}), neck_fwd:+neckAng.toFixed(1) }));
          }
        }
    ...
    if (current.id === "tpose") {
      ...
      if (ok && wristsUp) {
        ...
        setBaseline(b => ({
          ...(b ?? {}),
          shoulder_width_px: shoulderW!=null ? +shoulderW.toFixed(1) : b?.shoulder_width_px,
          wingspan_px:       wingspan !=null ? +wingspan .toFixed(1) : b?.wingspan_px,
        }));
        const upperArmL = toCm(dist(ls, lelb)), upperArmR = toCm(dist(rs, relb));
        const forearmL  = toCm(dist(lelb, lw)), forearmR  = toCm(dist(relb, rw));
        setLengths(prev => ({ ...(prev ?? {}), upperArmL, upperArmR, forearmL, forearmR }));

        const elbowLA = angleABC(ls, lelb, lw), elbowRA = angleABC(rs, relb, rw);
        const shAbdL  = angleToVertical(ls, lelb), shAbdR = angleToVertical(rs, relb);
        setAngles(a => ({
          ...(a ?? {}),
          elbowL: elbowLA != null ? +elbowLA.toFixed(1) : a?.elbowL,
          elbowR: elbowRA != null ? +elbowRA.toFixed(1) : a?.elbowR,
          shoulder_abd: (shAbdL!=null && shAbdR!=null)
            ? +(((shAbdL+shAbdR)/2).toFixed(1)) : a?.shoulder_abd,
        }));
    ...
    if (current.id === "side") {
      if (ls && rs && lh && rh && rk && ra) {
        const shoulderSpanN = Math.abs((kp[LM.LEFT_SHOULDER]?.x ?? 0) - (kp[LM.RIGHT_SHOULDER]?.x ?? 1));
        ...
        if (sideOK) {
          ...
          setAngles(a => ({
            ...
            ankle_side:ankleSide!=null ? +ankleSide.toFixed(1) : a?.ankle_side,
          }));
          setPosture(p => ({ ...(p ?? {}), trunk_fwd: trunkFwd ?? p?.trunk_fwd }));
    ...
    if (current.id === "ankle_rom") {
      ...
      if (ankleA!=null && !isNaN(ankleA)) {
        ...
        romTrack.current.ankle = { min: Math.min(t.min, ankleA), max: Math.max(t.max, ankleA) };
      }
    ...
    if (current.id === "shoulder_abd") {
      ...
      if (mean!=null) {
        ...
        romTrack.current.shoulder_abd = { min: Math.min(t.min, mean), max: Math.max(t.max, mean) };
      }
    ...
    if (current.id === "neck_rom") {
      if (le && re && ls && rs) {
        ...
        if (neckAng!=null) {
          ...
          romTrack.current.neck = { min: Math.min(t.min, neckAng), max: Math.max(t.max, neckAng) };
        }
  ...
  const finalizeStepRoms = () => {
    ...
    if (id === "shoulder_abd") {
      setRom(r => ({ ...(r ?? {}), shoulder_abd: diff(romTrack.current.shoulder_abd) }));
    }
    if (id === "neck_rom") {
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ border:"1px solid #e0e0e0", borderRadius:12, padding:16, background:"#fff" }}>
          ...
          <ul style={{ margin:0, paddingLeft:18, color:"#333", lineHeight:1.8 }}>
            <li>보정: <b>{cmPerPx ? `${cmPerPx.toFixed(4)} cm/px` : "-"}</b></li>
            <li>기준(어깨/윙스팬/골반): <b>
              {baseline?.shoulder_width_px ?? "-"}px / {baseline?.wingspan_px ?? "-"}px / {baseline?.pelvis_width_px ?? "-"}px
            </b></li>
            {circ && (circ.chest || circ.waist || circ.hip) && (
              <li>가슴/허리/엉덩이(마스크): <b>
                {circ?.chest ?? "-"} / {circ?.waist ?? "-"} / {circ?.hip ?? "-"} cm
              </b></li>
            )}
          </ul>
        </div>
